...
pub(crate) fn download_policy(
  ...
  unsafe {
    let request = match download.originalRequest() {
...
pub(crate) fn download_did_finish(this: &WryDownloadDelegate, download: &WKDownload) {
  unsafe {
    let Some(original_request) = download.originalRequest() else {
      return;
    };
    let Some(url) = original_request.URL().and_then(|u| u.absoluteString()) else {
      return;
    };
