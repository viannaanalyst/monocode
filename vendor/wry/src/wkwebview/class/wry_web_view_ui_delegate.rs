...
define_class!(
  ...
  unsafe impl WKUIDelegate for WryWebViewUIDelegate {
    ...
    unsafe fn create_web_view_for_navigation_action(
      ...
      // `define_class!` compiles the body into a function returning
      // `RetainedReturnValue`, so an early `return` cannot name this method's
      // return type. Run the body in a closure to be able to bail out early.
      (|| {
