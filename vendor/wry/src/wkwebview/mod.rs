...
pub fn url_from_webview(webview: &WKWebView) -> Result<String> {
  // WKWebView.URL() is nil until the first navigation commits. Unwrapping
  // panics the whole process (wry#1752). Empty string matches other platforms.
  let url_obj = match unsafe { webview.URL() } {
    Some(url) => url,
    None => return Ok(String::new()),
  };
  let absolute_url = match url_obj.absoluteString() {
    Some(url) => url,
    None => return Ok(String::new()),
  };
