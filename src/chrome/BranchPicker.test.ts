...
it("keeps branch names monospaced and the search in the interface typeface", async () => {
  act(() =>
    root.render(createElement(BranchPicker, { cwd: "/repo", branch: "main" })),
  );
  await act(async () => {});
  ...
  expect(search.className).not.toContain("font-mono");
  expect(branchName.className).toContain("font-mono");
