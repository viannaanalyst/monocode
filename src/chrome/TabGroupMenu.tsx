...
function MenuRow({
  ...
  return (
    <button
      ...
      className={`flex min-h-8 w-full items-center gap-2.5 rounded-lg px-2 text-left text-sm leading-none ${
        item.disabled
          ? "text-content/30"
          : item.danger
    ...
    >
      ...
      <span className={`min-w-0 flex-1 leading-label ${item.description ? "py-2" : "truncate"}`}>
        {item.label}
        {item.description ? (
          <span className="mt-1 block text-xs leading-snug text-content/60">
            {item.description}
          </span>
        ) : null}
      </span>
