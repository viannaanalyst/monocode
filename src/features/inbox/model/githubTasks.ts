...
import { jiraConnected, listJiraIssues, type JiraIssue } from "../../sessions/model/jira";
import {
  clickUpConnected,
  listClickUpTasks,
  type ClickUpTask,
} from "../../sessions/model/clickup";
import { notionConnected, listNotionTasks, type NotionTask } from "../../sessions/model/notion";
import {
...
export function invalidateGithubItem(
  ...
  for (const key of [...detailsByKey.keys()]) {
    if (key.endsWith(suffix)) detailsByKey.delete(key);
  }
  for (const key of [...threadByKey.keys()]) {
    if (key.endsWith(suffix)) threadByKey.delete(key);
  }
  for (const key of [...threadInflight.keys()]) {
    if (key.endsWith(suffix)) threadInflight.delete(key);
  }
  inboxListCache = null;
}
