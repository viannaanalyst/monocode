...
const INBOX_SOURCE_ORDER: ConnectableInboxSource[] = [
  "github",
  "linear",
  "gitlab",
  "jira",
...
export function visibleInboxSources(
  ...
  return INBOX_SOURCE_ORDER.filter(
    (source) => source in connections && connections[source] !== false,
  );
}
...
export function loadInboxConnections(): InboxSourceConnections {
  try {
    ...
    return {
      ...
      notion: connectFlag(record.notion),
