...
export function TranscriptFindBar({
  ...
  return (
    <div
      role="search"
      className="absolute right-3 top-2 z-30 flex items-center gap-1 rounded-lg border border-content/12 bg-background-base/95 px-1.5 py-1 shadow-xl backdrop-blur-xl"
      data-no-tooltip
    >
      <Search className="ml-0.5 size-3.5 shrink-0 text-content/40" />
      <input
        ref={inputRef}
        value={query}
        ...
        spellCheck={false}
        autoComplete="off"
        className="h-6 w-44 min-w-0 bg-transparent font-sans text-[12px] text-content outline-none placeholder:text-content/35"
      />
      <span
        aria-live="polite"
