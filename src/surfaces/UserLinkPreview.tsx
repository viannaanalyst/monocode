      ...
      <a
        ...
        className={`group mx-px inline-flex max-w-full items-center gap-1 rounded-md border-0 bg-transparent p-0 text-content/70 no-underline shadow-none outline-none transition-[color,transform] duration-[140ms] ease-[var(--motion-ease-out)] hover:text-content focus-visible:ring-2 focus-visible:ring-accent/60 active:scale-[0.98] motion-reduce:transition-none pb-px ${
          compact
            ...
            : "align-[-0.25em] text-2xs leading-4"
        }`}
...
function GithubWorkItemCard({
  ...
  return (
    <div className="min-w-0 text-left">
      ...
      {item ? (
        <>
          <h3 className="mt-2 line-clamp-2 text-xs font-semibold leading-[1.35] text-content">
            {item.title}
          </h3>
          {summary ? (
            <p className="mt-1.5 line-clamp-3 text-2xs leading-[1.45] text-content/55">
              {summary}
            </p>
          ) : null}
        </>
        ...
        <div className="mt-2">
          ...
          {summary ? (
            ...
            <p className="mt-1 text-2xs leading-relaxed text-content/50">
      ...
      {details?.author || updated ? (
        <div className="mt-2.5 flex min-w-0 items-center gap-1.5 text-2xs text-content/45">
          {details?.author ? (
            <GithubPerson
              name={details.author}
      ...
      {hasBranches ? (
        <div className="mt-2 flex min-w-0 items-center gap-1.5 rounded-md bg-content/[0.045] px-2 py-1.5 text-2xs text-content/50">
        ...
        <div className="mt-2.5 flex min-w-0 items-center gap-1.5">
          <div className="flex min-w-0 flex-1 flex-wrap gap-1">
            {labels.slice(0, 3).map((label) => (
              <span
                key={label.name}
                className="inline-flex max-w-28 items-center gap-1 rounded-full border border-content/10 bg-content/[0.035] px-1.5 py-0.5 text-2xs text-content/55"
              >
                <span
                  aria-hidden="true"
            ...
            {labels.length > 3 ? (
              <span className="px-1 text-2xs text-content/35">
                +{labels.length - 3}
              </span>
            ) : null}
          </div>
      ...
      <div className="mt-3 flex items-center gap-1.5 border-t border-content/[0.07] pt-2 text-2xs text-content/35">
