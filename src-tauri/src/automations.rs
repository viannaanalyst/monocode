...
/// The fork shipped its own automations module before upstream #327 landed.
/// Its tables reuse the same names with an incompatible column shape, so a
/// database from one of those builds is renamed here and re-imported by
/// [`import_legacy_fork_rows`] into the JSON-backed schema.
fn rename_legacy_fork_tables(conn: &Connection) -> rusqlite::Result<()> {
    if !table_exists(conn, "automations")?
        || table_has_column(conn, "automations", "definition_json")?
    {
        return Ok(());
    }
    if table_exists(conn, "automation_runs")? {
        conn.execute_batch(
            "ALTER TABLE automation_runs RENAME TO automation_runs_legacy_fork;
             ALTER TABLE automations RENAME TO automations_legacy_fork;",
        )?;
    } else {
        conn.execute_batch("ALTER TABLE automations RENAME TO automations_legacy_fork;")?;
    }
    Ok(())
}
...
fn import_legacy_fork_rows(conn: &Connection) -> rusqlite::Result<()> {
    if !table_exists(conn, "automations_legacy_fork")? {
        return Ok(());
    }
    ...
    let mut automations = Vec::new();
    {
        let mut statement = conn.prepare(
            "SELECT id, title, prompt, session_id, cwd, harness, model, runtime_mode,
                    model_settings_json, schedule_kind, weekday, hour, minute,
                    next_run_at, enabled, created_at, updated_at
             FROM automations_legacy_fork ORDER BY created_at ASC, id ASC",
        )?;
        ...
        for row in rows {
            ...
            let model_settings = serde_json::from_str::<serde_json::Value>(&model_settings_json)
                .ok()
            ...
            let schedule_kind = match schedule_kind.as_str() {
                "hourly" | "daily" | "weekdays" | "weekly" => schedule_kind,
                _ => "daily".into(),
            };
            let automation = Automation {
                ...
                reuse_session: session_id.is_some(),
                runtime_mode,
    ...
    if table_exists(conn, "automation_runs_legacy_fork")? {
        let mut statement = conn.prepare(
            "SELECT id, automation_id, scheduled_for, started_at, finished_at, status, error
             FROM automation_runs_legacy_fork ORDER BY scheduled_for ASC, id ASC",
        )?;
        let rows = statement.query_map([], |row| {
            Ok((
                row.get::<_, String>(0)?,
        ...
        for row in rows {
            let (id, automation_id, scheduled_for, started_at, finished_at, status, error) = row?;
            let status = match status.as_str() {
                "completed" => "succeeded".to_string(),
                "missed" | "skipped_busy" => "skipped".to_string(),
                other => other.to_string(),
            };
            runs.push(AutomationRun {
                id,
                automation_id,
                trigger: "time".into(),
                scheduled_for,
    ...
    for automation in &automations {
        let definition_json = serde_json::to_string(automation).map_err(to_sql_error)?;
        tx.execute(
    ...
    for run in &runs {
        let run_json = serde_json::to_string(run).map_err(to_sql_error)?;
        tx.execute(
            "INSERT INTO automation_runs (id, automation_id, created_at, run_json)
             VALUES (?1, ?2, ?3, ?4)",
            params![run.id, run.automation_id, run.created_at, run_json],
        )?;
    }
    tx.execute_batch(
        "DROP TABLE IF EXISTS automation_runs_legacy_fork;
         DROP TABLE IF EXISTS automations_legacy_fork;",
    )?;
    tx.commit()
}

pub(crate) fn ensure_tables(conn: &Connection) -> rusqlite::Result<()> {
    rename_legacy_fork_tables(conn)?;
    conn.execute_batch(
